...
void Scene_Title::CreateCommandWindow() {
        ...
        // "Exit Game" removed on purpose: do not push it to the options list,
        // and mark its index as unused (-1) so the vUpdate() handler above
        // never matches a real menu selection against it. This must happen
        // after all the conditional blocks above, so import/settings/translate
        // still get their correct, real indices.
        indices.exit = -1;
